...
function extractTimePatterns(rawText) {
  ...
  let maxSeconds = 0;
  let longThinkCount = 0; // 30秒以上の考慮
  let quickMoveCount = 0; // 5秒以内の着手
...
// iframe内やブラウザのセキュリティ制限環境でも確実にコピーを実行するハイブリッド関数
...
export default function MindBoardApp() {
  // モード：'adult'（一般・深層人生録） | 'kids'（こども・親子向け診断）
  ...
  const handlePasteClipboard = async () => {
    try {
      ...
      console.warn("Direct clipboard read blocked by browser", err);
    }
  };
...
以下のJSONスキーマに従って出力してください：
{
  ...
  "motto": "この人の生き様とこれからの歩みを照らす、心に刻むべきオリジナルの座右の銘"
    ...
    try {
      // ユーザーが設定したAPIキーがあればそちらを最優先し、なければデフォルトキーで試行
      ...
      console.warn("Gemini API連携エラー、ローカル深層診断エンジンへ移行:", err);
    }
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-3.5 flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2 font-sans text-xs">
            ...
            <button
              ...
              className={`p-1.5 rounded-xl border transition-colors flex items-center gap-1 text-xs ${
                customApiKey 
                  ? 'bg-emerald-50 border-emerald-300 text-emerald-800 font-medium' 
                  : 'bg-stone-50 hover:bg-stone-100 border-stone-200 text-stone-600'
              }`}
              title="ご自身のGemini APIキーを設定する（Web公開時用）"
            >
              <Key className="w-3.5 h-3.5 text-amber-600" />
              <span className="hidden sm:inline">{customApiKey ? 'APIキー接続中' : 'API設定'}</span>
            </button>
          </div>

        </div>
      </header>
      {/* JT杯・こども大会アピールバナー（こどもモード時のみ表示） */}
      {appMode === 'kids' && (
        <div className="bg-gradient-to-r from-amber-50 via-orange-50 to-stone-50 border-b border-orange-200/60 py-2.5 px-4 font-sans text-xs text-orange-950">
          <div className="max-w-6xl mx-auto flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              <span>
                <strong>JT将棋日本シリーズ・テーブルマークこども大会</strong>の対局記録や将棋教室の棋譜から、お子さんの「才能」「生きるチカラ」「心の豊かさ」を診断します。
              </span>
            </div>
            <div className="flex items-center gap-2">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="text-xs font-sans px-3 py-1.5 rounded-lg bg-amber-50 hover:bg-amber-100 text-amber-900 flex items-center gap-1.5 border border-amber-200 transition-colors"
                ...
                {pgnInput && (
                  <button
                    type="button"
                    onClick={handleClear}
                    className="text-xs font-sans px-2.5 py-1.5 rounded-lg text-stone-400 hover:text-stone-700 hover:bg-stone-100 transition-colors"
                  >
                    クリア
                  </button>
                )}
        ...
        <div className="fixed inset-0 bg-stone-900/60 backdrop-blur-xs z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-7 space-y-5 shadow-2xl border border-stone-200">
            <div className="flex items-center justify-between border-b border-stone-100 pb-3">
              <h4 className="font-bold text-stone-900 text-base font-sans flex items-center gap-2">
            ...
            <div className="space-y-3 font-sans text-xs text-stone-600 leading-relaxed">
              <p>
                Webアプリとしてご自身で公開される場合や、ご自身のGoogle AI StudioのAPIキーを使いたい場合は、以下に入力してください。
              </p>
              ...
              <div className="space-y-1.5 pt-1">
                ...
                <input
                  ...
                  placeholder="AIzaSy..."
                  className="w-full bg-[#FAF8F5] border border-stone-200 rounded-xl px-3.5 py-2.5 font-mono text-xs text-stone-800 focus:outline-hidden focus:border-amber-500"
                />
                <p className="text-[11px] text-stone-400">
                  ※ キーはブラウザ内の一時メモリにのみ保持され、外部サーバーに保存されることはありません。
                </p>
              </div>

              <div className="p-3.5 bg-amber-50 rounded-xl border border-amber-200/80 text-amber-900 space-y-1">
                <span className="font-bold">APIキーの取得方法（無料枠あり）：</span>
                <p className="text-[11px]">
                  Google AI Studio（<a href="https://aistudio.google.com/" target="_blank" rel="noreferrer" className="underline inline-flex items-center gap-0.5 font-medium">aistudio.google.com <ExternalLink className="w-2.5 h-2.5" /></a>）で「Get API key」から数クリックで即時発行できます。
                </p>
              </div>
            </div>

            <div className="flex justify-end gap-2 font-sans pt-2 border-t border-stone-100">
              {customApiKey && (
                <button
              ...
              <button
                onClick={() => setShowApiModal(false)}
                className="px-5 py-2.5 bg-amber-600 hover:bg-amber-700 text-white rounded-xl text-xs font-bold shadow-xs cursor-pointer"
              >
                設定を保存して閉じる
              </button>
            </div>
